...
DATASET_LIST = [MSRA, ALI,ICDAR2015,ICDAR2013,MLT]
#DATASET_LIST = [MLT]
MODEL_SAVE_PATH = './modelall1'
...
if __name__ == '__main__':
    ...
    for i in range(epoch):
        ...
        for im in train_im_list: # 每次只训练一张照片么？？？
            ...
            img, gt_txt = lib.dataset_handler.scale_img(img, gt_txt,1024)  # 图像缩放，保证最短边为600，标签也同步缩放
            ...
            tic = time.time()
            #IOU=np.zeros((tensor_img.shape[3],tensor_img.shape[2],10))
            try:
